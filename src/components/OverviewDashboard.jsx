...
const OverviewDashboard = () => {
    ...
    const handleDownloadReport = async () => {
        if (!activeTenantId) {
            alert("No active tenant selected context found.");
            return;
        }

        setIsDownloading(true);
        try {
            // Get generic MSAL Graph token for backend to proxy (override tenant context)
            const token = await getAccessToken({ 
                scopes: ['https://graph.microsoft.com/.default'],
                tenantId: activeTenantId 
            });
            
            const res = await fetch(`/api/admin/tenant-report/download?tenantId=${activeTenantId}`, {
                method: 'GET',
            ...
            a.download = `Tenant_Summary_Report_${activeTenantId}_${dateStr}.docx`;
            ...
            <header className="flex-between spacing-v-8">
                ...
                <div className="flex-gap-2">
                    <button 
                        ...
                        disabled={isDownloading || !activeTenantId}
                        style={{ padding: '8px 16px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px', opacity: (!activeTenantId || isDownloading) ? 0.6 : 1, cursor: (!activeTenantId || isDownloading) ? 'not-allowed' : 'pointer', fontWeight: 600 }}
                        title="Generate and download extensive tenant report via Graph API"
                    >
                        {isDownloading ? <RefreshCw size={16} className="spinning" /> : <Download size={16} />}
                        {isDownloading ? 'Generating...' : 'Download Summary Word Doc'}
                    </button>
                    <button
